...
def run(url):
    ...
    for x in range(len(url_date)):
        if url_date[x] != -1:
            urls = "http://www.qu.la/"+url_date[x]
            # page = page_title[x]
            try:
                threading.Thread(target=deal, args=[urls,x]).start()
                # deal(urls, x)
                time.sleep(2)
            except:
                pass
    # while error_list:
    #     print(error_list)
    #     _ = error_list.deepcopy()
    #     error_list = []
    #     for m in _:
    #         urls = m['url']
    #         num = m['num']
    #     try:
    #         threading.Thread(target=deal, args=[urls,num]).start()
    #         # deal(urls, x)
    #         time.sleep(1)
    #     except:
    #         pass   
...
def deal(url,x):
    try:
        ...
        print(x +"error")
        error_list.append({"url":url, "num":x})
